# Book example of a script to generate fake Intrusion Detection System warnings.
...
def ddosTest(src, dst, iface, count):
  ...
  send(pkt, iface=iface, count=count)
  pkt = IP(src=src,dst=dst)/ICMP(type=04,id=456)
  send(pkt, iface=iface, count=count)
...
def exploitTest(src, dst, iface, count):
  ...
  pkt = IP(src=src, dst=dst) / UDP(dport=635) \
        / Raw(load="^\xB0\x02\x89\x06\xFE\xC8\x89\x04\xB0\x06\x89")
  send(pkt, iface=iface, count=count)
...
def main():
  ...
  parser.add_option('-t', dest='tgt', type='string', help='sepcify target address')
